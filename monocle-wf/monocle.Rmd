...
cellranger_pipestance_path <- "../scrnaseq-wf/data/scrnaseq_samples/ovary_force_agg"
...
```
```{r plot.trajectory}
png('data/rough_trajectory.png', width = 1600, height = 1600, res = 150)
plot_cell_trajectory(dds_order, color_by = 'Cluster')
dev.off()
```

```{r output}
save(dds_order, file = 'data/monocle.Robj')
load('data/monocle.Robj')
```
...
write.table(cell.loc, file = 'data/cell_location.tsv', sep = '\t', quote = F, row.names = F)
...
write.table(tree.loc, file = 'data/tree_location.tsv', sep = '\t', quote = F, row.names = F)

branch.point <- dds_order@auxOrderingData[['DDRTree']]$branch_points
write(branch.point, file = 'data/branch_points.txt', sep = '\n')
...
write.table(metadata, file = 'data/pseudotime.tsv', sep = '\t', quote = F, row.names = F)
